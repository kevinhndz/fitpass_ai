...
SERVER_DOMAIN = os.environ.get("SERVER_DOMAIN", "159.65.223.239")
...
def _construir_url(cliente_id: int) -> str:
    ...
    return f"http://{SERVER_DOMAIN}:8080/validar/{cliente_id}"
...
def generar_qr(cliente_id: int, nombre: str, membresia: str, fecha_vencimiento) -> str:
    _asegurar_carpeta()
...
def enviar_qr_por_correo(destinatario: str, nombre: str, fecha_vencimiento, ruta_qr: str) -> bool:
    try:
...
def enviar_correo_recordatorio(destinatario: str, nombre: str, fecha_vencimiento, dias_restantes: int) -> bool:
    try:
        fecha_vencimiento = _parsear_fecha(fecha_vencimiento)
